...
La liaison est activée lorsqu’une application est intégrée `Release` à la configuration. Pour modifier cela, configurez la `BlazorWebAssemblyEnableLinking` propriété MSBuild dans le fichier projet :
...
Pour contrôler les assemblys I18N qui sont conservés, définissez `<BlazorWebAssemblyI18NAssemblies>` la propriété MSBuild dans le fichier projet :
