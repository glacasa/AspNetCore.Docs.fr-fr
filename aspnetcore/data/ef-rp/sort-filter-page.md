...
<span data-ttu-id="6a2b8-162">Pour vérifier que le tri fonctionne</span><span class="sxs-lookup"><span data-stu-id="6a2b8-162">To verify that sorting works:</span></span>
...
### <a name="iqueryable-vs-ienumerable"></a><span data-ttu-id="6a2b8-178">IQueryable et IEnumerable</span><span class="sxs-lookup"><span data-stu-id="6a2b8-178">IQueryable vs. IEnumerable</span></span>
...
<span data-ttu-id="6a2b8-191">Pour des raisons de performances, il est généralement préférable d’appeler `Contains` sur un `IQueryable`.</span><span class="sxs-lookup"><span data-stu-id="6a2b8-191">Calling `Contains` on an `IQueryable` is usually preferable for performance reasons.</span></span> <span data-ttu-id="6a2b8-192">Avec `IQueryable`, le filtrage est effectué par le serveur de base de données.</span><span class="sxs-lookup"><span data-stu-id="6a2b8-192">With `IQueryable`, the filtering is done by the database server.</span></span> <span data-ttu-id="6a2b8-193">Si un `IEnumerable` est créé en premier, toutes les lignes doivent être retournées à partir du serveur de base de données.</span><span class="sxs-lookup"><span data-stu-id="6a2b8-193">If an `IEnumerable` is created first, all the rows have to be returned from the database server.</span></span>
...
<span data-ttu-id="6a2b8-207">Testez l’application :</span><span class="sxs-lookup"><span data-stu-id="6a2b8-207">Test the app:</span></span>
...
### <a name="create-the-paginatedlist-class"></a><span data-ttu-id="6a2b8-222">Créer la classe PaginatedList</span><span class="sxs-lookup"><span data-stu-id="6a2b8-222">Create the PaginatedList class</span></span>
...
<span data-ttu-id="6a2b8-237">Tous les paramètres reçus par `OnGetAsync` sont Null si :</span><span class="sxs-lookup"><span data-stu-id="6a2b8-237">All the parameters that `OnGetAsync` receives are null when:</span></span>
...
## <a name="add-grouping"></a><span data-ttu-id="6a2b8-267">Ajouter la fonctionnalité de regroupement</span><span class="sxs-lookup"><span data-stu-id="6a2b8-267">Add grouping</span></span>
...
## <a name="add-sorting-to-the-index-page"></a><span data-ttu-id="6a2b8-292">Ajouter le tri à la page Index</span><span class="sxs-lookup"><span data-stu-id="6a2b8-292">Add sorting to the Index page</span></span>
...
## <a name="add-a-search-box-to-the-students-index-page"></a><span data-ttu-id="6a2b8-346">Ajouter une zone de recherche à la page d’index des étudiants</span><span class="sxs-lookup"><span data-stu-id="6a2b8-346">Add a Search Box to the Students Index page</span></span>
...
<span data-ttu-id="6a2b8-363">Par exemple, l’implémentation .NET Framework de `Contains` effectue par défaut une comparaison respectant la casse.</span><span class="sxs-lookup"><span data-stu-id="6a2b8-363">For example, the .NET Framework implementation of `Contains` performs a case-sensitive comparison by default.</span></span> <span data-ttu-id="6a2b8-364">Dans SQL Server, le respect de la casse de `Contains` est déterminé par le paramètre de classement de l’instance de SQL Server.</span><span class="sxs-lookup"><span data-stu-id="6a2b8-364">In SQL Server, `Contains` case-sensitivity is determined by the collation setting of the SQL Server instance.</span></span> <span data-ttu-id="6a2b8-365">Par défaut, SQL Server ne respecte pas la casse.</span><span class="sxs-lookup"><span data-stu-id="6a2b8-365">SQL Server defaults to case-insensitive.</span></span> <span data-ttu-id="6a2b8-366">`ToUpper` peut être appelée pour que le test ne respecte pas la casse de manière explicite :</span><span class="sxs-lookup"><span data-stu-id="6a2b8-366">`ToUpper` could be called to make the test explicitly case-insensitive:</span></span>
...
## <a name="add-paging-functionality-to-the-students-index-page"></a><span data-ttu-id="6a2b8-393">Ajouter la fonctionnalité de changement de page à la page d’index des étudiants</span><span class="sxs-lookup"><span data-stu-id="6a2b8-393">Add paging functionality to the Students Index page</span></span>
...
* <span data-ttu-id="6a2b8-421">La chaîne de recherche est changée.</span><span class="sxs-lookup"><span data-stu-id="6a2b8-421">The search string is changed.</span></span>
...
## <a name="update-the-about-page-to-show-student-statistics"></a><span data-ttu-id="6a2b8-443">Mettez à jour la page About pour afficher les statistiques sur les étudiants</span><span class="sxs-lookup"><span data-stu-id="6a2b8-443">Update the About page to show student statistics</span></span>

<span data-ttu-id="6a2b8-444">Lors de cette étape, nous allons mettre à jour *Pages/About.cshtml* afin d’afficher le nombre d’étudiants qui se sont inscrits pour chaque date d’inscription.</span><span class="sxs-lookup"><span data-stu-id="6a2b8-444">In this step, *Pages/About.cshtml* is updated to display how many students have enrolled for each enrollment date.</span></span> <span data-ttu-id="6a2b8-445">La mise à jour utilise le regroupement et comprend les étapes suivantes :</span><span class="sxs-lookup"><span data-stu-id="6a2b8-445">The update uses grouping and includes the following steps:</span></span>

* <span data-ttu-id="6a2b8-446">Créer un modèle de vue pour les données utilisées par la page **About**.</span><span class="sxs-lookup"><span data-stu-id="6a2b8-446">Create a view model for the data used by the **About** Page.</span></span>
* <span data-ttu-id="6a2b8-447">Mettre à jour la page About pour utiliser le modèle de vue.</span><span class="sxs-lookup"><span data-stu-id="6a2b8-447">Update the About page to use the view model.</span></span>

### <a name="create-the-view-model"></a><span data-ttu-id="6a2b8-448">Créer le modèle d’affichage</span><span class="sxs-lookup"><span data-stu-id="6a2b8-448">Create the view model</span></span>

<span data-ttu-id="6a2b8-449">Créez un dossier *SchoolViewModels* dans le dossier *Models*.</span><span class="sxs-lookup"><span data-stu-id="6a2b8-449">Create a *SchoolViewModels* folder in the *Models* folder.</span></span>

<span data-ttu-id="6a2b8-450">Dans le dossier *SchoolViewModels*, ajoutez un *EnrollmentDateGroup.cs* avec le code suivant :</span><span class="sxs-lookup"><span data-stu-id="6a2b8-450">In the *SchoolViewModels* folder, add a *EnrollmentDateGroup.cs* with the following code:</span></span>

[!code-csharp[](intro/samples/cu21/Models/SchoolViewModels/EnrollmentDateGroup.cs)]

### <a name="update-the-about-page-model"></a><span data-ttu-id="6a2b8-451">Mettre à jour le modèle de page About</span><span class="sxs-lookup"><span data-stu-id="6a2b8-451">Update the About page model</span></span>

<span data-ttu-id="6a2b8-452">Les modèles web dans ASP.NET Core 2.2 n’incluent pas la page About.</span><span class="sxs-lookup"><span data-stu-id="6a2b8-452">The web templates in ASP.NET Core 2.2 do not include the About page.</span></span> <span data-ttu-id="6a2b8-453">Si vous utilisez ASP.NET Core 2,2, créez la page about Razor .</span><span class="sxs-lookup"><span data-stu-id="6a2b8-453">If you are using ASP.NET Core 2.2, create the About Razor Page.</span></span>

<span data-ttu-id="6a2b8-454">Mettez à jour le fichier *Pages/About.cshtml.cs* avec le code suivant :</span><span class="sxs-lookup"><span data-stu-id="6a2b8-454">Update the *Pages/About.cshtml.cs* file with the following code:</span></span>

[!code-csharp[](intro/samples/cu21/Pages/About.cshtml.cs)]

<span data-ttu-id="6a2b8-455">L’instruction LINQ regroupe les entités Student par date d’inscription, calcule le nombre d’entités dans chaque groupe et stocke les résultats dans une collection d’objets de modèle de vue `EnrollmentDateGroup`.</span><span class="sxs-lookup"><span data-stu-id="6a2b8-455">The LINQ statement groups the student entities by enrollment date, calculates the number of entities in each group, and stores the results in a collection of `EnrollmentDateGroup` view model objects.</span></span>

### <a name="modify-the-about-razor-page"></a><span data-ttu-id="6a2b8-456">Modifier la page à propos de Razor</span><span class="sxs-lookup"><span data-stu-id="6a2b8-456">Modify the About Razor Page</span></span>

<span data-ttu-id="6a2b8-457">Remplacez le code du fichier *Pages/About.cshtml* par le code suivant :</span><span class="sxs-lookup"><span data-stu-id="6a2b8-457">Replace the code in the *Pages/About.cshtml* file with the following code:</span></span>

[!code-html[](intro/samples/cu21/Pages/About.cshtml)]

<span data-ttu-id="6a2b8-458">Exécutez l’application et accédez à la page About.</span><span class="sxs-lookup"><span data-stu-id="6a2b8-458">Run the app and navigate to the About page.</span></span> <span data-ttu-id="6a2b8-459">Le nombre d’étudiants pour chaque date d’inscription s’affiche dans une table.</span><span class="sxs-lookup"><span data-stu-id="6a2b8-459">The count of students for each enrollment date is displayed in a table.</span></span>

<span data-ttu-id="6a2b8-460">Si vous rencontrez des problèmes que vous ne pouvez pas résoudre, téléchargez [l’application terminée pour cette phase](https://github.com/dotnet/AspNetCore.Docs/tree/master/aspnetcore/data/ef-rp/intro/samples/StageSnapShots/cu-part3-sorting).</span><span class="sxs-lookup"><span data-stu-id="6a2b8-460">If you run into problems you can't solve, download the [completed app for this stage](https://github.com/dotnet/AspNetCore.Docs/tree/master/aspnetcore/data/ef-rp/intro/samples/StageSnapShots/cu-part3-sorting).</span></span>

![Page About](sort-filter-page/_static/about.png)

## <a name="additional-resources"></a><span data-ttu-id="6a2b8-462">Ressources supplémentaires</span><span class="sxs-lookup"><span data-stu-id="6a2b8-462">Additional resources</span></span>

* [<span data-ttu-id="6a2b8-463">Débogage d’une source ASP.NET Core 2.x</span><span class="sxs-lookup"><span data-stu-id="6a2b8-463">Debugging ASP.NET Core 2.x source</span></span>](https://github.com/dotnet/AspNetCore.Docs/issues/4155)
* [<span data-ttu-id="6a2b8-464">Version YouTube de ce tutoriel</span><span class="sxs-lookup"><span data-stu-id="6a2b8-464">YouTube version of this tutorial</span></span>](https://www.youtube.com/watch?v=MDs7PFpoMqI)

<span data-ttu-id="6a2b8-465">Dans le didacticiel suivant, l’application utilise des migrations pour mettre à jour le modèle de données.</span><span class="sxs-lookup"><span data-stu-id="6a2b8-465">In the next tutorial, the app uses migrations to update the data model.</span></span>

> [!div class="step-by-step"]
> <span data-ttu-id="6a2b8-466">[Précédent](xref:data/ef-rp/crud) 
>  [Suivant](xref:data/ef-rp/migrations)</span><span class="sxs-lookup"><span data-stu-id="6a2b8-466">[Previous](xref:data/ef-rp/crud)
