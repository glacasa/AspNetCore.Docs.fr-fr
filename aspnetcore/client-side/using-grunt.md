...
3. Dans le Explorateur de solutions, passez en revue la structure du projet. Le `\src` dossier comprend des `wwwroot` nœuds et `Dependencies` vides.
...
> [!NOTE]
> Si nécessaire, vous pouvez restaurer manuellement les dépendances dans **Explorateur de solutions** en cliquant avec le bouton `Dependencies\NPM` droit sur et en sélectionnant l’option de menu **restaurer les packages** .
...
1. À l' `initConfig` intérieur de la fonction, ajoutez `clean` les options de la tâche, comme indiqué dans l’exemple *Gruntfile. js* ci-dessous. La `clean` tâche accepte un tableau de chaînes de répertoire. Cette tâche supprime les fichiers de *wwwroot/lib* et supprime l’intégralité du répertoire */temp* .
...
1. Ajoutez la `jshint` tâche à l’aide du code ci-dessous.
    ...
    > [!NOTE]
    > L’option « -W069 » est une erreur produite par jshint lorsque JavaScript utilise la syntaxe des crochets pour assigner une propriété à la place `Tastes["Sweet"]` de la `Tastes.Sweet`notation par points, c’est-à-dire au lieu de. L’option désactive l’avertissement pour permettre au reste du processus de continuer.

1. Ajoutez la `uglify` tâche à l’aide du code ci-dessous.

    La tâche minimise le fichier *. js combiné* situé dans le répertoire temporaire et crée le fichier de résultats dans wwwroot/lib en suivant le nom * \<\>de fichier*de convention d’affectation de noms standard. min. js.
...
Ajoutez un appel à `loadNpmTasks()` pour afficher la `watch` tâche dans l’Explorateur de l’exécuteur de tâches.
