...
---
# <a name="debug-aspnet-core-no-locblazor-webassembly"></a>ASP.NET Core de débogage:::no-loc(Blazor WebAssembly):::

[Daniel Roth](https://github.com/danroth27)

:::no-loc(Blazor WebAssembly):::les applications peuvent être déboguées à l’aide des outils de développement de navigateur dans les navigateurs basés sur le chrome (Edge/chrome). Vous pouvez également déboguer votre application à l’aide de Visual Studio ou Visual Studio Code.
...
Pour activer le débogage d’une :::no-loc(Blazor WebAssembly)::: application existante, mettez à jour le `launchSettings.json` fichier dans le projet de démarrage pour inclure la `inspectUri` propriété suivante dans chaque profil de lancement :
...
* Permet à l’IDE de détecter que l’application est une :::no-loc(Blazor WebAssembly)::: application.
* Indique à l’infrastructure de débogage de script de se connecter au navigateur via le :::no-loc(Blazor)::: proxy de débogage de.
...
Pour déboguer une :::no-loc(Blazor WebAssembly)::: application dans Visual Studio :

1. Créez une nouvelle ASP.NET Core application hébergée :::no-loc(Blazor WebAssembly)::: .
...
Lors du débogage de votre :::no-loc(Blazor WebAssembly)::: application, vous pouvez également déboguer votre code serveur :
...
### <a name="debug-standalone-no-locblazor-webassembly"></a>Déboguer autonome:::no-loc(Blazor WebAssembly):::

1. Ouvrez l' :::no-loc(Blazor WebAssembly)::: application autonome dans vs code.
...
1. Quand vous y êtes invité, sélectionnez l’option de ** :::no-loc(Blazor WebAssembly)::: débogage** pour démarrer le débogage.
...
### <a name="debug-hosted-no-locblazor-webassembly"></a>Débogage hébergé:::no-loc(Blazor WebAssembly):::

1. Ouvrez le :::no-loc(Blazor WebAssembly)::: dossier de solution de l’application hébergée dans vs code.
...
Pour attacher une application en cours d’exécution :::no-loc(Blazor)::: , créez un `launch.json` fichier avec la configuration suivante :
...
| Option    | Description |
| --------- | ----------- |
| `request` | Utilisez `launch` pour lancer et attacher une session de débogage à une :::no-loc(Blazor WebAssembly)::: application ou `attach` pour attacher une session de débogage à une application déjà en cours d’exécution. |
...
| `hosted`  | Doit avoir la valeur en `true` cas de lancement et de débogage d’une application hébergée :::no-loc(Blazor WebAssembly)::: . |
...
#### <a name="launch-and-debug-a-standalone-no-locblazor-webassembly-app"></a>Lancer et déboguer une :::no-loc(Blazor WebAssembly)::: application autonome
...
#### <a name="launch-and-debug-a-hosted-no-locblazor-webassembly-app-with-microsoft-edge"></a>Lancer et déboguer une application hébergée :::no-loc(Blazor WebAssembly)::: avec Microsoft Edge
...
:::no-loc(Blazor):::fournit un proxy de débogage qui implémente le [protocole chrome devtools](https://chromedevtools.github.io/devtools-protocol/) et augmente le protocole avec. Informations spécifiques à .net. Quand le raccourci clavier de débogage est enfoncé, :::no-loc(Blazor)::: pointe le devtools chrome au niveau du proxy. Le proxy se connecte à la fenêtre du navigateur que vous cherchez à déboguer (par conséquent, il est nécessaire d’activer le débogage distant).

## <a name="browser-source-maps"></a>Mappages des sources du navigateur

Les mappages de source de navigateur permettent au navigateur de mapper les fichiers compilés à leurs fichiers sources d’origine et sont couramment utilisés pour le débogage côté client. Toutefois, :::no-loc(Blazor)::: ne mappe actuellement pas C# directement à JavaScript/WASM. Au lieu de cela, :::no-loc(Blazor)::: fait l’interprétation du langage intermédiaire dans le navigateur, les mappages de source ne sont donc pas pertinents.
