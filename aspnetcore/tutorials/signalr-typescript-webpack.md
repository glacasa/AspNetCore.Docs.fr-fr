...
1. Remplacez la `scripts` propriété du fichier *Package. JSON* par le code suivant :
...
1. Créez *src/index. TS* avec le code suivant :
    ...
    * `keyup`: Cet événement se déclenche lorsque l’utilisateur tape dans `tbMessage`la zone de texte. La fonction `send` est appelée quand l’utilisateur appuie sur la touche **Entrée**.
...
1. Ajoutez l’instruction `using` suivante en haut du fichier *Startup.cs* pour résoudre la `ChatHub` référence :
...
1. Ajoutez le code en surbrillance au fichier *src/index.ts* :
    ...
    SignalR permet l’échange de messages entre un client et un serveur. Chaque message a un nom spécifique. Par exemple, les messages portant le `messageReceived` nom peuvent exécuter la logique responsable de l’affichage du nouveau message dans la zone de messages. L’écoute d’un message spécifique peut être effectuée au moyen de la fonction `on`. Vous pouvez écouter n’importe quel nombre de noms de message. Vous pouvez aussi passer des paramètres au message, comme le nom de l’auteur et le contenu du message reçu. Dès que le client reçoit un message, un élément `div` est créé avec le nom de l’auteur et le contenu du message dans son attribut `innerHTML`. Le nouveau message est ajouté à l’élément `div` principal qui affiche les messages.
