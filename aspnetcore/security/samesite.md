...
## <a name="samesite-and-identity"></a>SameSite etIdentity
...
[HttpContext. Response. cookies. Append](xref:Microsoft.AspNetCore.Http.IResponseCookies.Append*) a pour valeur par défaut `Unspecified` , ce qui signifie qu’aucun attribut SameSite n’a été ajouté au cookie et que le client utilise son comportement par défaut (LAX pour les nouveaux navigateurs, None pour les anciens navigateurs). Le code suivant montre comment modifier la valeur du cookie SameSite en `SameSiteMode.Lax` :
...
* Toutes les API de cookies ont par défaut la valeur `Unspecified` . Certains composants qui utilisent des cookies définissent des valeurs plus spécifiques à leurs scénarios. Consultez le tableau ci-dessus pour obtenir des exemples.

::: moniker-end

::: moniker range=">= aspnetcore-3.0"

Dans ASP.NET Core 3,0 et versions ultérieures, les valeurs par défaut SameSite ont été modifiées pour éviter les conflits avec les paramètres par défaut incohérents du client. Les API suivantes ont modifié la valeur par défaut de à `SameSiteMode.Lax ` `-1` afin d’éviter d’émettre un attribut SameSite pour ces cookies :
...
* Spécifie les cookies qui déclarent explicitement `SameSite=None` afin d’activer la remise entre sites qui doit être marquée comme `Secure` . `None`nouvelle entrée à refuser.
...
Dans `Startup.Configure` , ajoutez le code qui appelle <xref:Microsoft.AspNetCore.Builder.CookiePolicyAppBuilderExtensions.UseCookiePolicy*> avant <xref:Microsoft.AspNetCore.Builder.AuthAppBuilderExtensions.UseAuthentication*> *d'* appeler ou toute méthode qui écrit des cookies :
...
### <a name="test-with-electron"></a>Tester avecElectron
