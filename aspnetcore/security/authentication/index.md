...
Si plusieurs schémas sont utilisés, les stratégies d’autorisation (ou attributs d’autorisation) peuvent [spécifier le schéma d’authentification (ou les schémas)](xref:security/authorization/limitingidentitybyscheme) dont ils dépendent pour authentifier l’utilisateur. Dans l’exemple ci-dessus, le schéma d’authentification de cookie peut être utilisé en`CookieAuthenticationDefaults.AuthenticationScheme` spécifiant son nom (par défaut, bien qu’un autre `AddCookie`nom puisse être fourni lors de l’appel de).

Dans certains cas, l’appel à `AddAuthentication` est automatiquement effectué par d’autres méthodes d’extension. Par exemple, lors de l’utilisation `AddAuthentication` de [ASP.net Core Identity ](xref:security/authentication/identity), est appelé en interne.

L’intergiciel d’authentification est ajouté dans `Startup.Configure` en appelant la <xref:Microsoft.AspNetCore.Builder.AuthAppBuilderExtensions.UseAuthentication*> méthode d’extension sur le de `IApplicationBuilder`l’application. L' `UseAuthentication` appel de enregistre l’intergiciel (middleware) qui utilise les schémas d’authentification précédemment enregistrés. Appelez `UseAuthentication` avant tout middleware dépendant des utilisateurs authentifiés. Lorsque vous utilisez le routage de point de `UseAuthentication` terminaison, l’appel à doit atteindre :
...
* Est dérivée <xref:Microsoft.AspNetCore.Authentication.IAuthenticationHandler> de <xref:Microsoft.AspNetCore.Authentication.AuthenticationHandler`1>ou.
...
* Un modèle de porteur JWT renvoyant un résultat 401 `www-authenticate: bearer` avec un en-tête.
