...
Le gestionnaire de secret est utilisé à partir d’une interface de commande ouverte à la [racine](xref:fundamentals/index#content-root)de `{SECRET NAME}` contenu du projet, `{SECRET VALUE}` où est le nom et est la valeur :
...
L’exemple d’application utilise un ID d’application et un certificat X. `#define` 509 lorsque l’instruction en haut du fichier *Program.cs* a la `Certificate`valeur.
...
* Valeurs hiérarchiques (sections) : utilisez `:` la notation (deux-points `GetSection` ) ou la méthode d’extension. Utilisez l’une des approches suivantes pour obtenir la valeur de configuration :
...
L’exemple d’application utilise des identités gérées pour `#define` les ressources Azure lorsque l’instruction en haut du fichier *Program.cs* a la valeur `Managed`.
...
* L' <xref:Microsoft.Azure.KeyVault.KeyVaultClient> instance est utilisée avec une implémentation par défaut <xref:Microsoft.Extensions.Configuration.AzureKeyVault.IKeyVaultSecretManager> de qui charge toutes les valeurs de secret et remplace les doubles`--`tirets () par`:`des signes deux-points () dans les noms de clés.
...
| `ReloadInterval` | `Timespan`pour attendre entre les tentatives d’interrogation du coffre de clés pour les modifications. La valeur par défaut `null` est (la configuration n’est pas rechargée). |
...
* `GetKey`:
  ...
  * Remplace deux tirets dans n’importe quel nom `KeyDelimiter`par le, qui est le délimiteur utilisé dans la configuration (généralement un signe deux-points). Azure Key Vault n’autorise pas les deux-points dans les noms de secrets.

[!code-csharp[](key-vault-configuration/samples_snapshot/Startup.cs)]

La `Load` méthode est appelée par un algorithme de fournisseur qui itère au sein des secrets du coffre pour trouver ceux qui ont le préfixe de version. Lorsqu’un préfixe de version est `Load`trouvé avec, l’algorithme utilise la `GetKey` méthode pour retourner le nom de configuration du nom de la clé secrète. Il supprime le préfixe de version du nom du secret et retourne le reste du nom de secret pour le chargement dans les paires nom-valeur de la configuration de l’application.
...
1. Si la version de l’application est modifiée dans le fichier projet `5.1.0.0` et que l’application est à nouveau exécutée, la valeur `5.1.0.0_secret_value_dev` secrète renvoyée se trouve `5.1.0.0_secret_value_prod` dans l’environnement de développement et en production.
...
Les secrets sont mis en `IConfigurationRoot.Reload()` cache jusqu’à ce que soit appelé. Les secrets ayant expiré, désactivés et mis à jour dans le coffre de clés ne `Reload` sont pas respectés par l’application tant qu’elle n’est pas exécutée.
...
Les secrets désactivés et <xref:Microsoft.Azure.KeyVault.Models.KeyVaultErrorException>arrivés à expiration lèvent un. Pour empêcher l’application de se déclencher, fournissez la configuration à l’aide d’un autre fournisseur de configuration ou mettez à jour le secret désactivé ou expiré.
...
* La stratégie d’accès n' `Get` inclut `List` pas les autorisations et.
