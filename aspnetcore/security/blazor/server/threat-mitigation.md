...
## <a name="blazor-and-shared-state"></a>Éblouissant et état partagé
...
* Tous les appels ont un délai d’expiration configurable après lequel ils échouent, en <xref:System.OperationCanceledException> retournant un à l’appelant.
  * Il y a un délai d’attente par défaut`CircuitOptions.JSInteropDefaultCallTimeout`pour les appels () d’une minute. Pour configurer cette limite, consultez <xref:blazor/call-javascript-from-dotnet#harden-js-interop-calls>.
...
* Traitez toute méthode .NET exposée à JavaScript comme vous le feriez pour un point de terminaison public de l’application.
  ...
  * Prenez en compte la possibilité d’exposer des méthodes statiques et d’instance à des clients JavaScript. Évitez de partager l’état entre les sessions, sauf si la conception appelle l’état de partage avec les contraintes appropriées.
    * Pour les méthodes d’instance `DotNetReference` exposées par le biais d’objets créés à l’origine par le biais de l’injection de dépendances, les objets doivent être inscrits en tant qu’objets délimités. Cela s’applique à tout service d’injection Blazor de services utilisé par l’application serveur.
...
* Évitez d’implémenter la distribution personnalisée des appels .NET en plus de l’implémentation de la distribution du Framework. L’exposition de méthodes .NET au navigateur est un scénario avancé, qui n’est pas Blazor recommandé pour le développement général.

### <a name="events"></a>Événements

Les événements fournissent un point d’entrée Blazor à une application serveur. Les mêmes règles de protection des points de terminaison dans les applications Web s’appliquent Blazor à la gestion des événements dans les applications serveur. Un client malveillant peut envoyer toutes les données qu’il souhaite envoyer en tant que charge utile d’un événement.
...
Certains événements DOM, tels que `oninput` ou `onscroll`, peuvent générer une grande quantité de données. Évitez d’utiliser ces événements Blazor dans les applications serveur.

## <a name="additional-security-guidance"></a>Aide supplémentaire sur la sécurité

Les conseils pour la sécurisation des applications Blazor ASP.net Core s’appliquent aux applications serveur et sont traités dans les sections suivantes :
...
Les interactions d’interopérabilité JS entre le client et le serveur sont enregistrées dans les <xref:Microsoft.Extensions.Logging.ILogger> journaux du serveur avec des instances. Blazorévite la journalisation des informations sensibles, telles que les événements réels ou les entrées et sorties d’interopérabilité JS.
...
* `DetailedErrors`clé de configuration. Par exemple, affectez `ASPNETCORE_DETAILEDERRORS` la valeur à la variable d' `true`environnement.
...
BlazorLe serveur SignalR utilise pour la communication entre le client et le serveur. BlazorLe serveur utilise normalement le transport SignalR qui négocie, qui est généralement WebSocket.
...
Les composants peuvent également déclencher des requêtes de navigation par programmation `NavigationManager`par le biais de l’utilisation de. Dans de tels scénarios, l’application peut effectuer une navigation côté client ou indiquer au navigateur d’accéder au nouvel emplacement.
...
Dans le cas contraire, un utilisateur malveillant peut forcer le navigateur à accéder à un site Contrôlé par un attaquant. Dans ce scénario, l’attaquant tente d’utiliser une entrée utilisateur dans le cadre de l’appel de la `NavigationManager.Navigate` méthode.
...
* Vérifiez que les paramètres CORS sont appropriés lors de l’activation de CORS Blazor ou de la désactivation explicite de cors pour les applications.
* Testez pour vous assurer que les limites côté serveur de Blazor l’application offrent une expérience utilisateur acceptable sans niveaux de risque inacceptable.
