...
Dans le code précédent, l’authentification de cookie et l’état de session définissent l’attribut sameSite sur `None` , en émettant l’attribut avec une `None` valeur, et en définissant également l’attribut Secure sur `true` .
